...
def apply_pca_dimensionality_reduction(
    ...
    if effective_n_components < 1: # [cite: 1]
    ...
    pca_model_path = config.PCA_MODEL_OUTPUT_PATH # [cite: 1]
    os.makedirs(os.path.dirname(pca_model_path), exist_ok=True) # [cite: 1]

    if dataset_type == "discovery":
        logging.info(f"Ajustando PCA con {effective_n_components} componentes en el conjunto 'discovery'...")
        try:
            reducer = PCA(n_components=effective_n_components, random_state=random_state) # [cite: 1]
            reduced_embeddings = reducer.fit_transform(embeddings) # [cite: 1]
            joblib.dump(reducer, pca_model_path) 
            logging.info(f"Modelo PCA ajustado y guardado en: {pca_model_path}")
            if hasattr(reducer, 'explained_variance_ratio_'): # [cite: 1]
...
def main_feature_engineering_pipeline(dataset_type_to_process: str):
    ...
    input_path = config.FE_INPUT_PATHS.get(dataset_type_to_process) # [cite: 1]
    embeddings_full_output_path = config.EMBEDDINGS_FULL_OUTPUT_PATHS.get(dataset_type_to_process) # [cite: 1]
    embeddings_ids_output_path = config.EMBEDDINGS_IDS_OUTPUT_PATHS.get(dataset_type_to_process) # [cite: 1]
    embeddings_reduced_output_path = config.EMBEDDINGS_REDUCED_OUTPUT_PATHS.get(dataset_type_to_process) # [cite: 1]

    if not all([input_path, embeddings_full_output_path, embeddings_ids_output_path]): # [cite: 1]
        ...
        return
    if config.FE_PERFORM_DIMENSIONALITY_REDUCTION and not embeddings_reduced_output_path: # [cite: 1]
    ...
    os.makedirs(config.FEATURES_DIR, exist_ok=True) # [cite: 1]
    ...
    try:
        df_preprocessed = pd.read_csv(input_path) # [cite: 1]
    ...
    if df_preprocessed.empty or config.COL_PREPROCESSED_TEXT not in df_preprocessed.columns: # [cite: 1]
        logging.error(f"DataFrame para '{dataset_type_to_process}' vacío o sin la columna '{config.COL_PREPROCESSED_TEXT}'.")
        return
    if config.COL_TWEET_ID not in df_preprocessed.columns: # [cite: 1]
    ...
    full_embeddings, tweet_ids = generate_embeddings( # [cite: 1]
        df=df_preprocessed,
        text_column=config.COL_PREPROCESSED_TEXT, # [cite: 1]
        id_column=config.COL_TWEET_ID, # [cite: 1]
        model_name=config.SENTENCE_TRANSFORMER_MODEL # [cite: 1]
    )

    if full_embeddings is None or tweet_ids is None or full_embeddings.size == 0: # [cite: 1]
        logging.error(f"Fallo en la generación de embeddings para '{dataset_type_to_process}'. Terminando esta etapa.")
        return

    save_data(full_embeddings, embeddings_full_output_path) # [cite: 1]
    save_ids_csv(tweet_ids, embeddings_ids_output_path, config.COL_TWEET_ID) # [cite: 1]

    # 2. Reducción de Dimensionalidad (controlada por config.py)
    if config.FE_PERFORM_DIMENSIONALITY_REDUCTION: # [cite: 1]
        if full_embeddings is not None and full_embeddings.size > 0: # [cite: 1]
            logging.info(f"Técnica de reducción configurada en config.py: '{config.FE_DIM_REDUCTION_TECHNIQUE.upper()}'") # [cite: 1]
            
            reduced_embeddings = None 
            if config.FE_DIM_REDUCTION_TECHNIQUE == 'pca': # [cite: 1]
            ...
            elif config.FE_DIM_REDUCTION_TECHNIQUE == 'umap': # [cite: 1]
            ...
            if reduced_embeddings is not None and reduced_embeddings.size > 0: # [cite: 1]
                save_data(reduced_embeddings, embeddings_reduced_output_path) # [cite: 1]
            else:
...
if __name__ == "__main__":
    ...
    if not config.SENTENCE_TRANSFORMER_MODEL: # [cite: 1]
        logging.critical("El modelo SENTENCE_TRANSFORMER_MODEL no está definido en config.py")
         ...
         not hasattr(config, 'PCA_MODEL_OUTPUT_PATH'): # Añadir validación para la nueva ruta
    ...
    elif config.FE_PERFORM_DIMENSIONALITY_REDUCTION and config.FE_DIM_REDUCTION_TECHNIQUE not in ['pca', 'umap']: # [cite: 1]
